...
const Dashboard = () => {
    ...
    const handleDeleteRow = async (rowId) => {
        try {
            await axios.delete(`http://localhost:8000/rapidops/api/htmlFile/deleteHtml/${rowId}`);
            const updatedTableData = tableData.filter(row => row.id !== rowId);
            setTableData(updatedTableData);
            setKey(prevKey => prevKey + 1);
        } catch (error) {
    ...
    // Filtering tableData based on search query
    const filteredTableData = tableData.filter((row) =>
        Object.values(row).some(
            (value) =>
                typeof value === 'string' &&
            ...
            <div className='content'>
                <div className='navBar' style={{border:'1px solid #E5E7EB'}}>
                    <div className='leftSideItems'>
                        <div>logo</div>
                        <div className='left'>
                            <span className="h4 font-weight-bold">Pages</span>
                            <span>create and publish pages</span>
                        </div>
                    </div>
                    <div className='rightSideItems'>
                        <button type="button" className="publishButton" onClick={() => navigate('/homePage')}>+ Add Page</button>
                    </div>
                </div>
                ...
                <div className='mainContent'>
                    <div className='searchRow'>
                        ...
                        <span>Entries: {filteredTableData.length}</span>
                        <select value={statusFilter} onChange={handleStatusChange}>
                            <option value="All">All</option>
                            <option value="draft">Draft</option>
                            <option value="published">Published</option>
                            ...
                            <tbody>
                                {filteredTableData.map((row, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className='tdtitle'>
                                            <div>{row.title}</div>
                                            <div className="dropdown" style={{ display:'flex', alignItems:'center', justifyContent:'center' }}>
                                                <button className="btn" type="button" id={`dropdownMenuButton-${index}`} data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">...</button>
                                                <div className="dropdown-menu" aria-labelledby={`dropdownMenuButton-${index}`}>
                                                    <a className="dropdown-item" href="#" onClick={() => handleDeleteRow(row._id)} style={{ color: 'red' }}>delete</a>
                                                    <a className="dropdown-item" href="#" onClick={() => handleEditRow(row._id)}>edit</a>
                                                </div>
                                            </div>
                                        </td>
                                        <td>{row.endPoint}</td>
